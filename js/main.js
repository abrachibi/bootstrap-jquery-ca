...
$(document).ready(function() {
    $(".faq-question").off("click").on("click", function(e) {
        ...
        const $answer = $(this).next(".faq-answer");
        const $icon = $(this).find(".toggle-icon"); // Target the Font Awesome caret

        if ($answer.is(":visible")) {
            $answer.stop(true, true).slideUp(400);
            $icon.removeClass("rotate-icon"); // Remove rotation
        } else {
            // Close others
            $(".faq-answer").stop(true, true).slideUp(400);
            $(".toggle-icon").removeClass("rotate-icon");
            ...
            // Open current
            $answer.stop(true, true).slideDown(400);
            $icon.addClass("rotate-icon"); // Add rotation
        }
    });
});
...
$(document).ready(function() {
    $('#numberAll').on('click', function() {
        ...
        // 2. TRIGGER THE TOAST (Offline BS 5.3 Style)
        // Select the raw DOM element instead of the jQuery object
        var toastEl = document.getElementById('numberToast');
        
        if (toastEl) {
            // Initialize and show using Bootstrap's static method
